...
# In this lab, we will learn how to obtain some whole-network (density, transitivity, diameter, etc.). In addition, we 
# will also learn how to detect sub-groups or communities within a given a network using different algorithms.  
...
# What did we just do? Change the class of the object
class(matrix_1)
matrix_1
class(m1)
m1
V(m1) # vertices or nodes
E(m1) # edges or ties
...
plot(m2,
     vertex.color = ifelse(attributes[attributes$matrix==2,]$gender=='female','yellow','lightblue'),
...
plot(m3,
     vertex.color = ifelse(attributes[attributes$matrix==3,]$gender=='female','yellow','lightblue'),
...
# EXERCISE: Do a 6-cluster partition of Network 1, and a 5-cluster partition of Network 3.
